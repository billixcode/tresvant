...
export function useUploadQueue() {
  ...
  const processNext = useCallback(async () => {
    ...
    try {
      ...
      const trackNumMatch = nameWithoutExt.match(/^(\d+)[\s._-]+(.+)/)
      const trackNumber = trackNumMatch ? parseInt(trackNumMatch[1], 10) : null
      const fallbackTitle = trackNumMatch ? trackNumMatch[2] : nameWithoutExt

      // Use file's lastModified as fallback recorded_date
      const lastMod = file.lastModified ? new Date(file.lastModified) : null
      const fallbackDate = lastMod ? lastMod.toISOString().substring(0, 10) : null
      ...
      if (trackNumber) insertData.track_number = trackNumber
      if (fallbackDate) insertData.recorded_date = fallbackDate
