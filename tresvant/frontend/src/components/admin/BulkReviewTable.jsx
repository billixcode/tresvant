...
import { Check, Trash2, Edit, ChevronDown, Sparkles, Loader } from 'lucide-react'
...
export default function BulkReviewTable({ tracks, onUpdate, onPublish, onDelete, people = [], onAiSuggest, onAiSuggestAll, aiLoadingIds = new Set() }) {
        ...
        <div className="flex items-center justify-between rounded-lg border border-neutral-700 bg-neutral-800 px-4 py-2">
          <span className="text-sm text-neutral-300">
            {aiLoadingIds.size > 0
              ...
              : `${tracks.length} track${tracks.length !== 1 ? 's' : ''} — use AI to fill in missing metadata`}
          </span>
          <button
            ...
            {tracks.map(track => (
              <tr key={track.id} className="hover:bg-neutral-800/50">
                ...
                <td className="px-3 py-2 w-10">
                  <InlineCell value={track.track_number} onChange={v => handleFieldUpdate(track.id, 'track_number', v ? parseInt(v, 10) : null)} />
                </td>
                <td className="px-3 py-2">
                ...
                <td className="px-3 py-2">
                  <InlineCell value={track.duration_secs ? `${Math.floor(track.duration_secs / 60)}:${String(Math.round(track.duration_secs % 60)).padStart(2, '0')}` : null} readOnly />
                </td>
